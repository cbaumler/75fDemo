 ...
 *
 * This file contains a dummy driver that configures the Atmel 
 * Mega ATXMEGA64B1 for SPI communication on port C as master. It also
 * provides interrupt processing rules for the SPI peripheral, 
 * provides a set of functions for reading and writing data to
 * a slave peripheral connected to the slave select on port C, and
 * provides a function to dump an initialization message to the SPI bus.
 * This driver uses a SPI driver to interface with the SPI module.
 */ 
